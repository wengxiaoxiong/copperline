...
import { SHOP, WEAPONS } from './inventory';
export function createShop() {
  ...
  ctx.font = '32px sans-serif'; ctx.fillText('3–15 金币 · 按 E / 商店按钮购买', 384, 123);
...
export function createExtraWeapons(parent: T.Group) {
  return WEAPONS.slice(1).map((_, index) => {
    const slot = index + 1;
    ...
    const length = slot === 1 ? .28 : slot === 2 ? .46 : slot === 4 ? 1.2 : .9;
    ...
    if (slot >= 4) box(.09, .14, .3, -.03, .1, grip);
    if (slot === 4) {
      const scope = new T.Mesh(new T.CylinderGeometry(.045, .045, .32, 12), steel);
      scope.rotation.x = Math.PI / 2; scope.position.set(0, .12, -.28); group.add(scope);
      box(.04, .08, .15, .065, -.28);
    }
    if (slot === 5) box(.18, .22, .2, -.15, -.25);
