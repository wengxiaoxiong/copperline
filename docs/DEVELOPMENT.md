...
## 2026-09-13 坡道路口弹跳与自行车速度修复

自行车真实沥青刚体起步回归：修复前持续踩踏 10 秒速度仅 0.316 m/s，修复零摩擦合并后为 11.964 m/s。坡道路口改用实际碰撞面支撑和平滑倾角，仅在贴地范围内跟随坡度，避免规划道路切换造成车身突变和保留坡顶上升速度。

`tests/driving-physics.test.ts` 调用真实 Game.step、Population 创建的四类载具和 Landscape 碰撞：默认种子最陡的 8 条道路两端共 64 次通过，检查四个车底角全部离开支撑面的间隙与行驶距离；另检查自由下落、撞墙和原点平移。64 次检查最大离地间隙为 0.401 米，阈值为 0.45 米；这是限定道路样本的回归，不是全地图无颠簸保证。

浏览器 Chrome 实际运行 `await (await import('/scripts/vehicle-driving-scenario.js')).vehicleDrivingScenario(window.__game)`：道路 15 路口四类载具均通过，车底离地间隙为 0.020–0.033 米；自行车 5 秒约 41.3 米，峰值约 12.04 m/s。已有 roadDrivingScenario 顺坡约 257 米、最陡坡反向上坡约 195 米、桥面约 219 米均通过。脚本会重置当前开发会话、清理车流并停止动画循环，刷新恢复。验证未覆盖完整键鼠、手机真机手感或持续帧率。

全量测试 61/61 通过，strict TypeScript 与 Vite 构建通过，保留既有 bundle 大小提示。改动未提交、推送或部署。
